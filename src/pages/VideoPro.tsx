...
import { saveVideoToGallery, getVideoGallery, deleteVideo } from "@/lib/api/video";
...
export default function VideoPro() {
  ...
  const [scenes,   setScenes]   = useState<Scene[]>([{ id: crypto.randomUUID(), text: "" }]);
  ...
  /* scenes */
  function addScene() { setScenes((p) => [...p, { id: crypto.randomUUID(), text: "" }]); }
  function removeScene(id: string) { setScenes((p) => p.filter((s) => s.id !== id)); }
  function updateScene(id: string, text: string) { setScenes((p) => p.map((s) => (s.id === id ? { ...s, text } : s))); }
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const url = await callGenerateVideo({
        ...
        style: "cinematic", duration: 10, ratio: "16:9", resolution: "1080p", mode: "text",
      });
      setVideoUrl(url);
      try {
  ...
  async function loadAiPrompts() {
    try {
      ...
      const res = await fetch(`${SUPABASE_URL}/functions/v1/prompt-ai`, {
        ...
        body: JSON.stringify({ prompt: prompt || "creative video prompts", type: "video" }),
      });
      const data = await res.json();
      const list = Array.isArray(data?.prompts) ? data.prompts : Array.isArray(data) ? data : [];
      setAiPrompts(list.slice(0, 3));
    } catch { setError("Failed to load AI prompts"); }
    finally { setIsLoadingAiPrompts(false); }
  }
  ...
  function applyPrompt(p: string) { setPrompt(p); setIsPromptPanelOpen(false); }
          ...
          <div className="max-w-5xl mx-auto w-full px-4 md:px-8 py-10">
            ...
            <div className="mb-8">
              <h1 className="text-2xl font-semibold tracking-tight flex items-center gap-2">
                Nexus Video Pro <Sparkles className="w-5 h-5 text-primary" />
              </h1>
              <p className="text-sm text-muted-foreground mt-1">
                Generate professional AI avatar videos using Nexus Video API.
                Choose your <strong>avatar</strong>, <strong>voice</strong> and add multiple scenes.
              </p>
            </div>
              ...
              <div className="flex-1 space-y-6">
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="relative rounded-2xl bg-card/90 px-4 py-4 md:px-6 md:py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="pointer-events-none absolute inset-x-0 -top-1 h-[2px] overflow-hidden">
                      <motion.div className="h-full w-[200%] bg-gradient-to-r from-violet-500 via-fuchsia-500 to-violet-500"
                        animate={{ x: ["0%", "-50%"] }} transition={{ repeat: Infinity, duration: 8, ease: "linear" }} />
                    </div>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold flex items-center gap-2">
                          Nexus Video Pro
                          <span className="inline-flex items-center rounded-full bg-violet-500/15 px-2 py-0.5 text-[10px] font-medium text-violet-200 border border-violet-500/40">LIVE</span>
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">AI avatars, voice selection, multi-scene scripts and AI Prompt Assistant.</p>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-[11px]">
                      <div className="rounded-xl border border-violet-500/30 bg-violet-950/40 px-3 py-2 text-violet-50">
                        <div className="flex items-center gap-1.5 mb-0.5"><User className="w-3 h-3 text-violet-300" /><span className="font-medium">AI Avatars</span></div>
                        <p className="text-[10px] text-violet-100/80">Choose from realistic avatar styles.</p>
                      </div>
                      <div className="rounded-xl border border-violet-500/30 bg-violet-950/40 px-3 py-2 text-violet-50">
                        <div className="flex items-center gap-1.5 mb-0.5"><Mic className="w-3 h-3 text-violet-300" /><span className="font-medium">Voice + Scenes</span></div>
                        <p className="text-[10px] text-violet-100/80">Pick voice and write multi-scene scripts.</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {TEMPLATES.map((t) => (
                      <button key={t.value} onClick={() => { setTemplate(t.value); if (t.prompt) setPrompt(t.prompt); }}
                        className={cn("px-3 py-1.5 rounded-full text-xs font-medium border transition-all",
                          template === t.value ? "bg-violet-600 text-white border-violet-500 shadow-glow" : "bg-card text-muted-foreground border-border hover:border-violet-500/60 hover:text-foreground")}>
                        {t.label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-medium text-muted-foreground flex items-center gap-2">
                    Prompt
                    <span className="rounded-full bg-muted px-2 py-[2px] text-[10px] text-muted-foreground border border-border">Describe your video</span>
                  </label>
                  ...
                  <div className="flex justify-end">
                    <button onClick={togglePromptPanel}
                      ...
                      {isPromptPanelOpen ? <PanelRightClose className="w-3.5 h-3.5" /> : <PanelRightOpen className="w-3.5 h-3.5" />}
                      AI Prompt
                    </button>
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <p className="text-[11px] font-medium text-muted-foreground flex items-center gap-1.5"><User className="w-3 h-3" /> Select Avatar</p>
                    <div className="grid grid-cols-3 gap-2">
                      {AVATARS.map((a) => (
                        <button key={a} onClick={() => setAvatar(a)}
                          className={cn("rounded-2xl border aspect-square flex flex-col items-center justify-center gap-1 text-[11px] font-medium transition-all",
                            avatar === a ? "bg-violet-600 text-white border-violet-500 shadow-glow" : "bg-card text-muted-foreground border-border hover:border-violet-500/60 hover:text-foreground")}>
                          <User className="w-5 h-5" />{a}
                        </button>
                      ))}
                    </div>
                  </div>
                  <div className="space-y-2">
                    <p className="text-[11px] font-medium text-muted-foreground flex items-center gap-1.5"><Mic className="w-3 h-3" /> Select Voice</p>
                    <div className="flex flex-wrap gap-2">
                      {VOICES.map((v) => (
                        <button key={v} onClick={() => setVoice(v)}
                          className={cn("px-3 py-1.5 rounded-full text-xs font-medium border transition-all",
                            voice === v ? "bg-violet-600 text-white border-violet-500 shadow-glow" : "bg-card text-muted-foreground border-border hover:border-violet-500/60 hover:text-foreground")}>
                          {v}
                        </button>
                      ))}
                    </div>
                ...
                <div className="space-y-3">
                  <p className="text-[11px] font-medium text-muted-foreground flex items-center gap-1.5"><Video className="w-3 h-3" /> Scenes</p>
                  <div className="space-y-2">
                    {scenes.map((scene, idx) => (
                      <div key={scene.id} className="flex items-center gap-2">
                        <span className="text-[11px] text-muted-foreground w-14 shrink-0">Scene {idx + 1}</span>
                        <input value={scene.text} onChange={(e) => updateScene(scene.id, e.target.value)}
                  ...
                  <button onClick={addScene} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-violet-300 transition-colors">
                ...
                <div className="mt-4">
                  <div className="rounded-2xl border border-border bg-card/60 min-h-[260px] flex items-center justify-center overflow-hidden relative">
                    {isGenerating ? (
                      ...
                      <motion.video key={videoUrl} src={videoUrl} controls
                        initial={{ opacity: 0, scale: 1.02 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }}
                      ...
                      <div className="flex flex-col items-center justify-center text-center px-6 py-10 text-muted-foreground text-xs gap-2">
                        ...
                        <p className="font-medium text-foreground">No video yet</p>
                  ...
                  {videoUrl && (
                    <div className="mt-2 flex justify-center">
                      <a href={videoUrl} download className="text-xs text-violet-400 hover:underline">Pobierz wideo</a>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-8">
                  ...
                  {isLoadingGallery ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {gallery.map((vid) => (
                        <div key={vid.id} className="relative group rounded-2xl overflow-hidden border border-border bg-card">
                          <video src={vid.url} className="w-full h-32 object-cover" />
                          <button
                            onClick={async () => { await deleteVideo(vid.id, vid.url); setGallery((p) => p.filter((v) => v.id !== vid.id)); }}
                  ...
                  <motion.aside
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="text-xs font-semibold text-violet-50">AI Prompt Assistant</p>
                          <p className="text-[10px] text-violet-200/80">Curated + AI‑generated prompts</p>
                        </div>
                      </div>
                      <button onClick={() => setIsPromptPanelOpen(false)}
                    ...
                    <div className="space-y-3 overflow-y-auto pr-1 text-[11px]">
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-violet-300/80 mb-1.5">Curated prompts</p>
                        <div className="space-y-1.5">
                          {STATIC_PROMPTS.map((p, idx) => (
                            <button key={idx} onClick={() => applyPrompt(p)}
                      ...
                      <div className="pt-2 border-t border-violet-500/20">
                        <div className="flex items-center justify-between mb-1.5">
                          <p className="text-[10px] uppercase tracking-wide text-violet-300/80">AI‑generated prompts</p>
                          <button onClick={loadAiPrompts} disabled={isLoadingAiPrompts}
                            ...
                            {isLoadingAiPrompts ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                            Refresh
                          </button>
                        </div>
